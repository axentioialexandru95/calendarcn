...
const appDir = path.join(workRoot, "starter-consumer")
const smokePort = process.env.CALENDARCN_SMOKE_PORT ?? "3200"
...
runCheckedCommand(
  ...
  "Failed to create the starter consumer app with create-next-app"
)
...
runCheckedCommand(
  ...
  "Failed to initialize shadcn in the starter consumer app"
)
...
runCheckedCommand(
  ...
  "Failed to build registry output before the starter consumer smoke test"
)
...
try {
  runCheckedCommand(
    ...
    [
      ...
      "shadcn",
      "add",
      `${server.baseUrl}/calendarcn.json`,
      "--cwd",
      appDir,
      "--yes",
      "--overwrite",
    ...
    "Failed to install CalendarCN into the starter consumer app"
  )
} finally {
...
runCheckedCommand(
  ...
  "Failed to install dependencies after adding CalendarCN to the starter consumer app"
)
...
runCheckedCommand(
  ...
  "Failed to build the starter consumer app after installing CalendarCN"
)

console.log("Starter consumer smoke app is ready.")
...
console.log("- CalendarCN starter install from a local registry server")
...
function writeSmokeTestPage(filePath) {
  writeFileSync(
    ...
    [
      ...
      "            CalendarCN starter consumer harness",
      "          </h1>",
      '          <p className="max-w-3xl text-sm leading-6 text-muted-foreground">',
      "            This app exists only to verify that CalendarCN installs cleanly into a fresh",
      "            external Next.js project and renders a usable scheduler surface without any",
      "            local repo aliases or hidden wiring.",
      "          </p>",
      "        </header>",
      "",
      "        <CalendarScheduler",
